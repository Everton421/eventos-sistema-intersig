...
import { dispathExchange } from "../broker/producers/producer.ts";
  ...
  export async function serviceVerifyPedidosEvent(){
setInterval( async ()=>{
             const [rows  , fields ] = await dbConn.query(`SELECT * FROM ${databaseEventos}.eventos_pedidos_sistema where STATUS = 'PENDENTE' LIMIT 100 ;`);
          ...
          for(const i of data ){
               const id_message = randomUUID() as string;
                const resultMsg =  await dispathExchange(
                    ...
                    if(resultMsg){
                         console.log(`[X] Enviado Evento Pedido | RabbitMQ: id ${id_message}`)
                              const sql = `UPDATE ${databaseEventos}.eventos_pedidos_sistema SET status = 'PROCESSADO', id_message = '${id_message}' WHERE id = ${i.id}  ;` 
                     try {
