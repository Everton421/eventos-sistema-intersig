...
export async function serviceVerifyProdutctEvent() {

     setInterval(async () => {
          const [rows, fields] = await dbConn.query(`SELECT * FROM ${databaseEventos}.eventos_produtos_sistema where STATUS = 'PENDENTE' LIMIT 100 ;`);
          ...
          if (data.length > 0) {
               ...
               for (const i of data) {
                    ...
                    if (resultMsg) {
                         ...
                         const sql = `UPDATE ${databaseEventos}.eventos_produtos_sistema SET status = 'PROCESSADO', id_message = '${id_message}' WHERE id = ${i.id}  ;`
                         try {
